...
class DateUtilizator {
  ...
  static List<Map<String, dynamic>> sporturiAzi = [];
  ...
  static Future<void> salveazaDate() async {
    ...
    await prefs.setString('sporturiAzi', json.encode(sporturiAzi));
  }
  ...
  static Future<void> incarcaDate() async {
    ...
    if (prefs.containsKey('ultimaData')) {
      ...
      if (ultimaDataAccesata != dataDeAzi) {
        ...
        if (prefs.containsKey('sporturiAzi')) {
          List<dynamic> lista = json.decode(prefs.getString('sporturiAzi')!);
          sporturiAzi = lista.map((e) => e as Map<String, dynamic>).toList();
        }
      }
    } else {
      ultimaDataAccesata = dataDeAzi;
